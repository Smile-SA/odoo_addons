...
class BudgetLine(models.Model):
    ...
    @api.multi
    def _get_sql_query(self, analytic_account_id, date_from, date_to, acc_ids, commitment=False):
        """
        If computing commitment, search lines with commitment account, else with general account.
        """
        sql_string = "SELECT SUM(amount) "\
                     ...
                     "AND " + \
                     ("%s " % "commitment_account_id" if commitment else "general_account_id") + \
                     "=ANY(%s)"
    ...
    def _prac_amt(self, commitment=False):
        ...
        for line in self:
            ...
            if not acc_ids:
                raise UserError(_("The Budget '%s' has no accounts!") % tools.ustr(line.general_budget_id.name))
            ...
            if line.analytic_account_id.id:
                sql_string, sql_args = self._get_sql_query(line.analytic_account_id.id, date_from, date_to, acc_ids, commitment)
...
class BudgetPositionCommitmentLimit(models.Model):
    ...
    budget_post_id = fields.Many2one('account.budget.post', 'Budgetary Position', required=True, index=True, ondelete='cascade')
    ...
    def check_amount_limit_inferior_global_limit(self):
        if self.user_id.commitment_global_limit and self.amount_limit > self.user_id.commitment_global_limit:
            raise UserError(_("You cannot define a budget post commitment limit superior to the global limit of this user"))
