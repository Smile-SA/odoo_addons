...
class AnalyticLine(models.Model):
    ...
    commitment_type = fields.Selection([
        ...
        ('pay', 'Pay commitment'),
    ], 'Commitment type', help="Analytic line generated from sale, purchase or pay")
    commitment_account_id = fields.Many2one('account.account', string='Commitment Account', ondelete='restrict', readonly=True,
    ...
    def _check_budget_availability(self):
        if self.budget_line_id and self.budget_line_id.available_amount < 0.0:
            raise UserError(_("Available amount [%s%s] is exceeded for the budget line '%s'")
                            % (abs(self.budget_line_id.available_amount),
                               self.budget_line_id.company_id.currency_id.symbol, self.display_name))

    @api.one
    @api.constrains('user_id', 'amount', 'account_id', 'general_account_id', 'date')
