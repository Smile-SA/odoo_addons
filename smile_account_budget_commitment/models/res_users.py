        ...
        if limits:
            for limit in limits:
                if limit.amount_limit < abs(amount):
                    raise UserError(warning_msg % limit.budget_post_id.display_name)
        elif self.commitment_global_limit and self.commitment_global_limit < abs(amount):
            raise UserError(warning_msg % self.env['account.budget.post'].browse(budget_post_id).display_name)
        return True
