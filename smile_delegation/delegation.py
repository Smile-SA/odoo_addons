...
def launch_delegation_decorations(original_method):
    ...
    def wrapper(self, cr, module):
        ...
        if delegation_tmpl_obj and hasattr(delegation_tmpl_obj, 'decorate_delegated_methods'):
            cr.execute("SELECT relname FROM pg_class WHERE relname=%s", (delegation_tmpl_obj._table,))
            if cr.fetchall():
...
class DelegationTemplate(osv.osv):
    ...
    def decorate_delegated_methods(self, cr):
        ...
        methods_to_decorate = [(delegation_tmpl.model, delegation_tmpl.method)
                               for delegation_tmpl in self.browse(cr, 1, delegation_tmpl_ids)]
        for model, method_name in list(set(methods_to_decorate)):
            class_obj = self.pool.get(model)
            if not class_obj:
                continue
    ...
    def __init__(self, pool, cr):
        ...
        setattr(Registry, 'load', launch_delegation_decorations(getattr(Registry, 'load')))
