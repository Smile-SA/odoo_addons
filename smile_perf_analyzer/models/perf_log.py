...
class IrLoggingPerfLog(models.Model):
    ...
    def _get_user(self):
        for rec in self:
            rec.user_id = self.env['res.users'].browse(rec.uid)
    ...
    def _format_db_stats_in_html(self):
        for rec in self:
            db_stats = safe_eval(rec.db_stats)
            data = []
            for key in db_stats:
                table, statement = key
            ...
            rec.db_stats_html = self._format_in_html(data, header)

    def _format_slow_queries_in_html(self):
        for rec in self:
            data = safe_eval(rec.slow_queries)
            data = sorted(data, key=itemgetter(1), reverse=True)
            header = _('Slow Query'), _('Duration'), _('Trace')
            rec.slow_queries_html = self._format_in_html(data, header)

    def _format_slow_recomputation_in_html(self):
        for rec in self:
            data = safe_eval(rec.slow_recomputation)
            data = sorted(data, key=itemgetter(2), reverse=True)
            header = _('Model'), _('Field'), _('Duration'), _('Count')
            rec.slow_recomputation_html = self._format_in_html(data, header)
