...
class IrLoggingPerfLog(models.Model):
    ...
    def _create_table(self):
        ...
        try:
            ...
            pass
        columns = [(k, f.column_type[1]) for k, f in self._fields.iteritems() if f.column_type]
        with db_connect(perf_dbname).cursor() as new_cr:
            new_cr.execute("SELECT relname FROM pg_class WHERE relkind IN ('r','v') AND relname=%s",
                           (self._table,))
            if not new_cr.rowcount:
                new_cr.execute('CREATE TABLE "%s" (id SERIAL NOT NULL, PRIMARY KEY(id))' % self._table)
            for f, t in columns:
                if f == 'id':
                    continue
    ...
    def _table_exist(self):
        perf_dbname = self._cr.dbname + '_perf'
        query = "SELECT datname from pg_database WHERE datname='%s'" % perf_dbname
        self._cr.execute(query)
        if not self._cr.rowcount:
            return False
        query = "SELECT relname FROM pg_class WHERE relkind IN ('r','v') AND relname=%s"
        with db_connect(self._cr.dbname + '_perf').cursor() as new_cr:
            new_cr.execute(query, (self._table,))
