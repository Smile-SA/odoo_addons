...
class ImportDataRun(osv.osv):
    ...
    def init_from_config_file(self, cr, uid, ids, context=None):
        ...
        for run in self.browse(cr, uid, ids, context):
            ...
            tools.convert_yaml_import(cr, 'smile_import_data', file(yaml_file), mode='init', noupdate=True)
        return True
    ...
    def genertate_template_file(self, cr, uid, ids, context):
        for obj in self.browse(cr, uid, ids, context):
            ...
            with open(file_name, 'wb') as new_file:
                ...
                csv_file.writerow(common_field_names)
                csv_file.writerow(descriptions)
...
#                    model_module = ir_md.read(cr, uid, imd_field_ids[0], ['module'], context)['module']
        ...
        for obj in self.browse(cr, uid, ids, context):
            try :
                ...
                if not attended_file_ids:
                    obj.write({'file_missed_ok':True}, context)
                    continue
                ...
                if not res:
                    continue
                obj.write({'validation_succeed_ok':True}, context)
            except Exception, e:
    ...
    def import_data(self, cr, uid, ids, context=None):
        ...
        '''
        # Start  validation
        res = self.start_file_validation(cr, uid, ids, context)
        if not res :
            self.log(cr, uid, ids, 'File validation process failed', context)
            return False
...
ImportDataRunObjectValidationField()
