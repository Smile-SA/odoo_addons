...
class ResPartner(models.Model):
    ...
    parent_type_ids = fields.Many2many('res.partner.type', string='Company types authorized for parent',
                                       compute='_get_parent_types')
    contact_ids = fields.One2many('res.partner', 'parent_id', 'Contacts & Addresses',
                                  domain=[('is_company', '=', False)])
    subcompanies_count = fields.Integer('Number of sub-companies', compute='_count_subcompanies')
    ...
    def _onchange_company_type(self):
        ...
        self.partner_type_id = self.partner_type_id.search([('code', '=', code)], limit=1)
    ...
    def _get_inherit_values(self, partner_type, not_null=False):
        if not partner_type:
            return {}
        inherit_fields = getattr(partner_type, '_%s_inherit_fields' % partner_type.company_type)
    ...
    def _update_children(self, vals):
        for partner in self:
            if partner.child_ids and partner.partner_type_id.field_ids:
                children_vals = {key: value for key, value in vals.items()
    ...
    def _update_fields_view_get_result(self, result, view_type='form'):
        if view_type == 'form' and not self._context.get('display_original_view'):
            ...
            for node in doc.xpath("//field[@name='child_ids']"):
                ...
                node.set('modifiers', json.dumps({'default_customer': False, 'default_supplier': False}))
                result['fields']['contact_ids'] = result['fields']['child_ids']
                result['fields']['contact_ids'].update(self.fields_get(['contact_ids'])['contact_ids'])
            result['arch'] = etree.tostring(doc)
        return result

    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(ResPartner, self).fields_view_get(view_id, view_type, toolbar, submenu)
        return self._update_fields_view_get_result(res, view_type)

    @api.model
    def _format_args(self, args):
        for cond in (args or []):
            if len(cond) == 3 and cond[2] and isinstance(cond[2], list) and isinstance(cond[2][0], list):
    ...
    def _get_display_name_context(self):
        self.ensure_one()
        partner = self.with_context(show_address=None, show_address_only=None, show_email=None)
    ...
    def _compute_display_name(self):
        ...
        if rule:
            self.display_name = safe_eval(rule, self._get_display_name_context())
        else:
