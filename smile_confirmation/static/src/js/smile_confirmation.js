odoo.define('smile_confirmation.smile_confirmation', function (require) {
"use strict";
    ...
    FormController.include({
        ...
        _onSave: function (ev) {
            ...
            var record_id = (record && record.data && record.data.id) ? record.data.id : false
                            ...
                            Dialog.confirm(self, popup_values[index].message, {
                                ...
                                confirm_callback: async () => {
                                    ...
                                    for (var j=0; j<field_names.length; j++){
                                        datas.push({
                                            name: field_names[j],
                                            value: (changes && changes[field_names[j]]) ? changes[field_names[j]] : ((record.data[field_names[j]]=='') ? record.data[field_names[j]] : false)
                                        })
                                    }
                                    index++;
                                    if (popup_values.length > index){
                                        ...
                                        self.rpc_call(modelName, 'execute_processing', record_id, datas);
                                        save();
                                    }
                                }
            ...
            self.rpc_call(modelName, 'get_message_informations', record_id, changes==null ? {} : changes).then(function(results){
