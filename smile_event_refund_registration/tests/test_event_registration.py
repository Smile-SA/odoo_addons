...
class TestEventRegistration(TransactionCase):
    ...
    def setUp(self):
        ...
        self.registration = self.EventRegistration.search(
            [('sale_order_id.name', '=', self.sale_order.name)])
    ...
    def test_01_cancel_and_refund_event_registration_to_cancel(self):
        ...
        """
        for record in self.registration:
            record.button_cancel_refund()
            self.assertEqual(record.state, 'cancel', 'Registration should be cancel')
    ...
    def test_02_cancel_and_refund_event_registration_to_refund(self):
        ...
        for record in self.registration:
            record.button_cancel_refund()
            self.assertEqual(
                record.state, 'refund', 'Registration should be Refund')
    ...
    def test_03_cancel_and_refund_event_to_cancel(self):
        ...
        self.assertEqual(
            event_refund.event_id.stage_id.name, 'Cancelled', 'Event should be Cancel')
