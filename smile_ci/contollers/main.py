...
class CiController(Controller):
    ...
    def get_build_infos(self, repo_id, branch, kpi):
        ...
        builds = Build.search_read(
            domain, ['state', kpi, last_update], limit=1)
        if not builds:
            return {}
        return builds[0]
    ...
    def badge(self, repo_id, branch, theme='default'):
        # Inspired by
        # https://github.com/odoo/odoo-extra/blob/master/runbot/runbot.py
    ...
    @route([
        ...
        '/smile_ci/badge/tests/<any(default,flat):theme>/<int:repo_id>/'
        '<branch>.svg',
    ], type="http", auth="public", methods=['GET', 'HEAD'])
    def badge_tests(self, repo_id, branch, theme='default'):
        ...
        return self.render_badge(
            theme, 'Failed tests', failed_test_ratio, color, build)

    @route([
        '/smile_ci/badge/quality/<int:repo_id>/<branch>.svg',
        '/smile_ci/badge/quality/<any(default,flat):theme>/<int:repo_id>/'
        '<branch>.svg',
    ], type="http", auth="public", methods=['GET', 'HEAD'])
    def badge_quality(self, repo_id, branch, theme='default'):
        ...
        if build['state'] == 'testing':
            ...
            if quality_code_count:
                color = '#e05d44'
        return self.render_badge(
            theme, 'Quality errors', quality_code_count, color, build)

    @route([
        '/smile_ci/badge/coverage/<int:repo_id>/<branch>.svg',
        '/smile_ci/badge/coverage/<any(default,flat):theme>/<int:repo_id>/'
        '<branch>.svg',
    ], type="http", auth="public", methods=['GET', 'HEAD'])
