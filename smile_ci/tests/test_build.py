...
from odoo.addons.smile_docker.tools import cursor
...
class TestBuild(TransactionCase):
    ...
    def setUp(self):
        ...
        self.branch = self.env.ref(
            'smile_ci.scm_repository_branch_smile_addons_90')
        pending_builds = self.branch.build_ids.filtered(
            lambda build: build.state == 'pending')
        if pending_builds:
    ...
    def test_010_create_build(self):
        ...
        with cursor(self.cr.dbname, False) as new_cr:
            current_builds = self.branch.with_env(
                self.env(cr=new_cr)).build_ids
