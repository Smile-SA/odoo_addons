...
class ir_model_export(osv.osv):
    ...
    def _get_last_attachments(self, cr, uid, ids, name, args, context=None):
        ...
        for export in self.read(cr, uid, ids, ['report_id']):
            ...
            attachment_ids = self.pool.get('ir.attachment').search(cr, uid, [
                ('res_model', '=', self._name), ('res_id', '=', export['id']),
            ], limit=1, order='create_date desc')
            if attachment_ids:
                res[export['id']]['attachment_id'] = attachment_ids[0]
            if export['report_id']:
                exceptions_attachment_ids = self.pool.get('ir.attachment').search(cr, uid, [
                    ('res_model', '=', 'res.request'), ('res_id', '=', export['report_id'][0]),
                ], limit=1, order='create_date desc')
    ...
    _columns = {
        'export_file_template_id': fields.related('export_tmpl_id', 'export_file_template_id',
                                                  type='many2one', relation='ir.model.export.file_template',
                                                  string='File Template', readonly=True),
