...
class IrModelExportTemplate(models.Model):
    ...
    def _get_filtered_res_ids(self, model_obj, *args):
        """
        Returns the ids of records matching the filter.
        :param model_obj: The model object.
        :param args: Additional arguments for the filter method.
        """
        if self.filter_type == 'domain':
            return self._get_res_ids_from_domain(model_obj)
        return self._get_res_ids_from_method(model_obj, *args)

    def _get_res_ids_from_domain(self, model_obj):
        """
        Returns the ids of records matching the domain.
        :param model_obj: The model object.
        """
        domain = safe_eval(
    ...
    def _get_res_ids_from_method(self, model_obj, *args):
        """
        Returns the ids of records matching the method.
        :param model_obj: The model object.
        :param args: Additional arguments for the method.
        """
        if not (
                self.filter_method and hasattr(model_obj, self.filter_method)
        ):
            raise UserError(
                _(
    ...
    def _get_already_exported_res_ids(self):
        """
        Returns the ids of records that have already been exported.
        """
        return set(
            sum(
                [
