...
class AuditRule(models.Model):
    ...
    def _activate(self):
        if self._context and self._context.get("activation_in_progress"):
            return
    ...
    @api.model
    def _register_hook(self, ids=None):  # noqa: CCR001
        self = self.sudo()
        updated = False
        if ids:
        ...
        for rule in rules:
            if (
                rule.model_id.model not in self.env.registry.models
                or not rule.active
            ):
                continue
    ...
    def create(self, vals_list):
        rules = super(AuditRule, self).create(vals_list)
        for rule in rules:
            rule.update_rule()
    ...
    @classmethod
    def _format_data_to_log(cls, old_values, new_values):  # noqa: CCR001
        data = {}
        for age in ("old", "new"):
        ...
        for res_id in list(data.keys()):
            ...
            for field in all_fields:
                if data[res_id]["old"].get(field) == data[res_id]["new"].get(
                    field
                ):
