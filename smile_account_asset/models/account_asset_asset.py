...
class AccountAssetAsset(models.Model):
    ...
    def _get_default_uom(self):
        return self.env.ref(
            'product.product_uom_unit', raise_if_not_found=False).id
    ...
    def _compute_depreciation_lines(self, depreciation_type='accounting'):
        ...
        if self.depreciation_line_ids and \
                kwargs['board_stop_date'] < \
                self.depreciation_line_ids[-1].depreciation_date:
        ...
        for depreciation_date in list(kwargs['readonly_values'].keys()):
            expr = "'%s' %s '%s'" % (depreciation_date, operator, max_date[:7])
            if safe_eval(expr):
    ...
    def _get_regularization_tax_rate(self):
        ...
        if self.purchase_tax_amount:
            ...
            if apply_regulatization and 0.0 <= \
                    int(time.strftime('%Y')) - int(self.purchase_date[:4]) < \
                ...
                remaining_years = regularization_period - (
                    int(self.sale_date[:4]) -
                    int(depreciation_start_date[:4]) + 1)
