...
CREATE AGGREGATE public.last (
    ...
    def _get_line_ids_from_depreciation_lines(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context):
            if line.depreciation_type != 'exceptional':
                res.extend([l.id for l in line.asset_id.fiscal_depreciation_line_ids if not l.is_posted])
        return list(set(res))
    ...
    _columns = {
        ...
        'accounting_value': fields.function(_get_fiscal_depreciation_info, method=True, type='float', store={
            'account.asset.depreciation.line': (_get_line_ids_from_depreciation_lines, ['asset_id', 'depreciation_value',
                                                                                        'depreciation_type', 'depreciation_date'], 5),
        }, digits_compute=dp.get_precision('Account'), string='Accounting Depreciation', multi='fiscal_depreciation'),
        'accelerated_value': fields.function(_get_fiscal_depreciation_info, method=True, type='float', store={
            'account.asset.depreciation.line': (_get_line_ids_from_depreciation_lines, ['asset_id', 'depreciation_value',
                                                                                        'depreciation_type', 'depreciation_date'], 5),
        }, digits_compute=dp.get_precision('Account'), string='Accelerated Depreciation', multi='fiscal_depreciation'),
    ...
    def _validate_fields(self, cr, uid, ids, fields_to_validate, context=None):
        ...
        if context.get('no_validate'):
            return
        super(AccountAssetDepreciationLine, self)._validate_fields(cr, uid, ids, fields_to_validate, context)
