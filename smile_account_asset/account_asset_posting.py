...
class AccountAssetAsset(orm.Model):
    ...
    def _get_move_lines(self, cr, uid, asset_id, journal_type, amount_excl_tax, tax_amount, tax_ids, accounts=None, default=None, context=None):
        lines = []
    ...
    def output(self, cr, uid, ids, context=None):
        if isinstance(ids, (int, long)):
            ids = [ids]
        for asset in self.browse(cr, uid, ids, context):  # Constater les amortissements/dérogations non encore comptabilisés
        ...
        accelerated_value = exceptional_value = 0.0
        context = context or {}
        ...
        for asset in self.browse(cr, uid, ids, context):
            ...
            for line in asset.fiscal_depreciation_line_ids:
                if not line.move_id:
                    continue
                accelerated_value += line.accelerated_value
            for line in asset.exceptional_depreciation_line_ids:
                if not line.move_id:
                    continue
                exceptional_value += line.depreciation_value
            if exceptional_value:  # Annuler les dépréciations (exceptional_value)
                context['force_account_move_amount'] = exceptional_value * -1
                asset.exceptional_depreciation_line_ids[-1].post_depreciation_line()
            if accelerated_value:  # Annuler les amortissements dérogatoires (accelerated_value)
