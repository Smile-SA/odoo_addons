...
from depreciation_board import DepreciationBoard, get_period_start_days, get_period_start_date


def _get_depreciation_date(depreciation_date, prorata=True, months=0, output='datetime'):
    relative_args = {'months': months}
    if not prorata:
        relative_args['day'] = 1
    return datetime.strptime(depreciation_date, '%Y-%m-%d') + relativedelta(**relative_args)
...
class AccountAssetAsset(orm.Model):
    ...
    def _get_depreciation_infos(self, cr, uid, ids, name, arg, context=None):
        ...
        fields_to_read = ['purchase_value', 'salvage_value', 'depreciation_date_start', 'sale_value', 'state', 'period_length', 'accounting_prorata']
        fields_to_read += ALL_DEPRECIATION_FIELDS
        for asset in self.read(cr, uid, ids, fields_to_read, context):
            ...
            res[asset['id']] = {
                'depreciation_date_stop': _get_depreciation_date(asset['depreciation_date_start'], asset['accounting_prorata'],
                                                                 asset['accounting_periods'] * asset['period_length']
                                                                 ).strftime('%Y-%m-%d'),
    ...
    _columns = {
        ...
        'accounting_method': fields.selection(DEPRECIATION_METHODS, 'Computation Method', required=True, readonly=True,
                                              states={'draft': [('readonly', False)]}),
        'accounting_periods': fields.integer('Number of Depreciations', required=False, readonly=True,
                                             states={'draft': [('readonly', False)]}),
        ...
        'fiscal_method': fields.selection(DEPRECIATION_METHODS, 'Computation Method', required=True, readonly=True,
                                          states={'draft': [('readonly', False)]}),
        'fiscal_periods': fields.integer('Number of Depreciations', required=False, readonly=True, states={'draft': [('readonly', False)]}),
        ...
        'disposal_receivable_account_id': fields.related('category_id', 'disposal_receivable_account_id', type='many2one', relation='account.account',
                                                         string='Disposal Receivable Account', readonly=True, store=True),
    }
    ...
    def _check_depreciation_length(self, cr, uid, ids, context=None):
        if isinstance(ids, (int, long)):
            ids = [ids]
        for asset in self.browse(cr, uid, ids, context):
            if asset.accounting_periods * asset.period_length % 12:
                return False
        return True
    ...
    def _compute_depreciation_lines(self, cr, uid, asset_id, depreciation_type='accounting', context=None):
        ...
        period_start_days = get_period_start_days(fiscalyear_start_day, asset.period_length)
        prorata = getattr(asset, '%s_prorata' % depreciation_type)
        readonly_values = {}
        exceptional_values = {}
        for line in asset.depreciation_line_ids:
            period_start_month = get_period_start_date(line.depreciation_date, period_start_days).strftime('%Y-%m')
            if line.depreciation_type == depreciation_type and line.move_id:
                readonly_values.setdefault(period_start_month, 0.0)
                readonly_values[period_start_month] += line.depreciation_value
            elif line.depreciation_type == 'exceptional':
                exceptional_values.setdefault(period_start_month, 0.0)
                exceptional_values[period_start_month] += line.depreciation_value
        board = DepreciationBoard(asset.purchase_value, method, periods, degressive_rate, prorata, asset.salvage_value,
                                  asset.depreciation_date_start, readonly_values, exceptional_values,
                                  fiscalyear_start_day, asset.period_length, asset.accounting_periods,
                                  rounding=len(str(asset.company_id.currency_id.rounding).split('.')[-1]),
                                  disposal_date=asset.sale_date or None)
        asset = self.browse(cr, uid, asset_id, context)
        for line in board.compute():
            vals = line.__dict__
    ...
    def _get_book_value_at_disposal(self, cr, uid, asset, context=None):
        ...
        if asset.accounting_depreciation_line_ids:
            book_value = asset.accounting_depreciation_line_ids[-1].book_value_wo_exceptional
        return book_value
