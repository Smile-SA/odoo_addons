...
class AccountMove(orm.Model):
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        ...
        if move_ids_to_reverse:
            ...
            if invoice_to_cancel_ids:
                ...
                reversed_move_ids = self.create_reversals(cr, uid, move_ids_to_reverse, reversal_date, post_and_reconcile=True)
    ...
    def unlink(self, cr, uid, ids, context=None, check=True):
        ...
        if isinstance(ids, (int, long)):
            ids = [ids]
        for move in self.read(cr, uid, ids, ['to_be_reversed'], context):
            if not move['to_be_reversed']:
