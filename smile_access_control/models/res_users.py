...
class ResUsers(models.Model):
    ...
    def _compute_share(self):
        for user in self:
            user.share = user.is_user_profile or not user.has_group(
                'base.group_user')
    ...
    user_ids = fields.One2many('res.users', 'user_profile_id', 'Users', domain=[
    ...
    def _update_from_profile(self, fields=None):
        ...
        if len(self.mapped('user_profile_id')) != 1:
            raise UserError(
                _("_update_from_profile accepts only users linked to a same profile"))
        ...
        if user_profile:
            ...
            for field in fields:
                ...
                if field_type == 'many2one':
                    ...
                    raise UserError(
                        _("_update_from_profile doesn't manage fields.One2many"))
                else:
                    vals[field] = value
            if vals:
                self.write(vals)
