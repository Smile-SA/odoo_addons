...
@api.multi
def new_store_set_values(self, fields):
    if not self._context.get('store_in_secure_mode'):
        return native_store_set_values(self, fields)
    res_ids_in_error = {}
    for res in self:
        try:
            native_store_set_values(res, fields)
        except Exception, e:
            res_ids_in_error[res.id] = e
            continue
