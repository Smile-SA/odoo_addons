    ...
    * Get upgrades to apply to database
    ...
    def __enter__(self):
        if config.get('no_upgrade_lock'):
            return self
        #  Lock upgrade to avoid concurrent upgrade in multi front
        #  configuration
        unlocked = False
        while not unlocked:
    ...
    def set_db_version(self):
        ...
        _logger.debug(
            "database version updated to %s", self.code_version)
...
class Upgrade(object):
    ...
    * Post-load: accept .sql, .csv, .xml and .py files
    ...
    def _manage_file_import_errors(
            self, error_management, cr, fname, e
    ):
