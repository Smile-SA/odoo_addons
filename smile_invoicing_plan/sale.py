...
class sale_order(osv.osv):
        ...
        if not order.contract_ok :
            ...
            if partner_lines :
                ...
                if inv_id :
                    ...
                    self.pool.get('account.invoice.line').write(cr,uid,partrner_lines,{'invoice_id':inv_id})
                else:
    ...
    def action2_invoice_create(self, cr, uid, ids, grouped=False, states=['confirmed', 'done', 'exception'], date_inv = False, context=None):
        ...
        for sale_order in self.browse(cr,uid,ids):
            inv_ids = self.action_invoice_create(cr,uid,[sale_order.id],grouped,states,date_inv,context)
