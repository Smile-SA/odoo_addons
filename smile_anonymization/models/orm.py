...
def _new_field_create(self, cr, context=None):
    ...
    - update ir_model_fields with the fields found in _columns
     (there is some redundancy as _columns is updated from
    ...
    if not cr.rowcount:
        ...
        cr.execute("INSERT INTO ir_model (id,model, name, info,state) VALUES (%s, %s, %s, %s, %s)",
    ...
    for (k, f) in model_fields:
        ...
        if getattr(f, 'serialization_field', None):
            ...
            if not serialization_field_id:
                raise except_orm(_('Error'), _("Serialization field `%s` not found for sparse field `%s`!") %
                                 (f.serialization_field, k))
        ...
        if k not in cols:
            ...
            cr.execute("""INSERT INTO ir_model_fields (
                ...
                relation,view_load,state,select_level,relation_field, translate, serialization_field_id, data_mask,data_mask_locked
            ) VALUES (
                ...
                if cols[k][key] != vals[key]:
                    ...
                    cr.execute("""UPDATE ir_model_fields SET
                        ...
                        view_load=%s, select_level=%s, readonly=%s ,required=%s, selectable=%s, relation_field=%s,
                         translate=%s, serialization_field_id=%s,
                        data_mask=%s, data_mask_locked=%s
                    WHERE
                        model=%s AND name=%s""", (
                            vals['model_id'], vals['field_description'], vals['ttype'],
                            vals['relation'], bool(vals['view_load']),
                            vals['select_level'], bool(vals['readonly']), bool(vals['required']),
                            bool(vals['selectable']), vals['relation_field'],
                            bool(vals['translate']), vals['serialization_field_id'], vals['data_mask'],
                            vals['data_mask_locked'], vals['model'], vals['name']
                        ))
                    break
