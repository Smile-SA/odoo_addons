...
class WebsiteAttachmentPage(http.Controller):
    ...
    def get_attachments(self):
        attachment_ids = request.env['ir.attachment'].sudo().search([('website_published', '=', True)])
        documents = {}
        for record in attachment_ids:
            document_type = record.document_type_id and record.document_type_id.name or _('Others')
    ...
    def attachment_details(self, **kw):
        return request.render('smile_publish_document.attachment_template', {'attachments': self.get_attachments()})

    @http.route(['/attachment/download'], type='http', auth='public')
    def download_attachment(self, attachment_id, **kw):
        fields = ["datas_fname", "datas", "file_type", "res_model", "res_id", "type", "url"]
        attachment = request.env['ir.attachment'].sudo().search_read([('id', '=', int(attachment_id))], fields)
        if not attachment:
