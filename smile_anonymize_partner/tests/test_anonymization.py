...
class TestAnonymization(TransactionCase):
    ...
    def invalidate_anonymized_record(self, partner):
        ...
        partner.invalidate_recordset(
            ...
            + anonymized_fields["phones"]
            + anonymized_fields["emails"]
        )
    ...
    def test_anonymization_on_enabled_partner(self):
        ...
        context = partner.with_user(self.user).action_anonymization()[
            "context"
        ]
        anonymize_partner_wizard = (
            self.env["confirm.anonymization"]
            .with_context(active_ids=context.get("active_ids"))
            .create({})
        )
