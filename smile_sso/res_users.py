...
class ResUsersExpiry(orm.Model):
    ...
    def __init__(self, pool, cr):
        ...
        try:
            self._duration = int(config_options.get('smile_sso.lifetime_duration'))
        except:
            self._duration = int(60*60*24*365)
...
class ResUsers(orm.Model):
    ...
    def check(self, db, uid, passwd):
        ...
        try:
            cr.autocommit(True)
            if self._res_users__uid_cache.get(db, {}).get(uid) != passwd:
                ...
                if not res:
                    ...
                    # raise OpenERPException(error_msg, ('', '', ''))
                self._res_users__uid_cache.setdefault(db, {}).update({uid: passwd})
