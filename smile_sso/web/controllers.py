...
def _get_connection_info(req, **kwargs):
    ...
    for k in kwargs:
        if k == 'db':
            db = kwargs[k]
    # Try to get username from Apache Variable. If there is no REMOTE_USER
    # variable in the request, you would probably need the below in your
    ...
    try:
        # Try to get username part of REMOTE_USER
        login = req.httprequest.headers.get('Remote-User').partition('@')[0]
    except:
        login = ''
...
def sso_login(self, req, **kwargs):
    ...
    if db and login and security_key:
        ...
        # Redirect to standard login page if can't do SSO
        addr = redirect('/', 303)
        cookie_val = urllib2.quote(simplejson.dumps(req.session_id))
        addr.set_cookie('instance0|session_id', cookie_val)
        return addr
    return redirect(sso_portal)
