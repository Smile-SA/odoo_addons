...
class BaseModel(models.AbstractModel):
    ...
    def _get_webservice_call(
        ...
        webservice_call = self.env.context.get('webservice_call')
        if not webservice_call:
            datas = self._prepare_webservice_call(
                webservice_type, type_request, name, url,
    ...
    def _generate_webservice_authenticate(self):
        ...
        login, password = (
            authentication_data.get('login'),
            authentication_data.get('password')
        )
