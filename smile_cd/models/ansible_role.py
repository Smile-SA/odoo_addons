...
class AnsibleRole(models.Model):
    ...
    tags = fields.Char('Default tags')
    is_odoo = fields.Boolean()
    package_ids = fields.One2many('scm.version.package', 'ansible_role_id',
                                  'Supported Odoo versions/Operating systems', copy=False)
    ...
    def download_sources(self, remove_if_exists=False):
        for role in self:
            ...
            if directory_exists:
                role.vcs_id.pull(self.directory)
            else:
