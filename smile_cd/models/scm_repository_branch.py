...
class Branch(models.Model):
    ...
    ansible_deployment_ids = fields.One2many(
        'ansible.deployment', 'branch_id', 'Deployments', readonly=True)
    ansible_deployments_count = fields.Integer(
        'Deployments count', compute='_get_ansible_deployments_count')
    use_branch_tmpl_to_deploy = fields.Boolean(
        'Use template to deploy', default=True)
    ansible_deployment_rule_ids = fields.One2many(
        'ansible.deployment.rule', 'branch_id',
        'Automatic deployment rules', context={'active_test': False})
    ...
    def update_ansible_inventories(self):
        ...
        for inventory_type in self.ansible_inventory_type_ids:
            ...
            if package.ansible_role_id and \
                    package.ansible_role_id not in existing_roles:
                roles_to_add.append(get_role_infos(package))
            for docker_image in self.mapped('link_ids.linked_image_id'):
                if docker_image.ansible_role_id and \
                        docker_image.ansible_role_id not in existing_roles:
            ...
            self.write({'ansible_inventory_ids': [
                       (0, 0, inventory_vals)
                       for inventory_vals in roles_to_add]})
        return True
    ...
    def open_deployment_wizard(self):
        ...
        return {
            ...
            'view_id': self.env.ref(
                'smile_cd.view_ansible_deployment_popup').id,
            'res_id': False,
            'domain': [],
            'target': 'new',
