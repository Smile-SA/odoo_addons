...
class Checklist(models.Model):
    ...
    model_id = fields.Many2one(
        'ir.model', 'Model', required=True, auto_join=True, ondelete='cascade')
    ...
    def _compute_progress_rates(self, records=None):
        for checklist in self:
            if not checklist._context.get('do_not_compute_progress_rates'):
                if not records:
                    records = self.env[checklist.model].sudo(). \
                ...
                for record in records.with_context(active_test=True):
                    ...
                    if vals:
                        ...
                        record.write(vals)
                        if checklist.action_id and old_progress_rate != \
    ...
    def _get_record_vals(self, record):
        ...
        for task_inst in task_insts:
            task_inst.complete = bool(record.filtered_domain(
                safe_eval(task_inst.task_id.complete_domain)))
                    ...
                    <button class="o_button_icon oe_stat_button"
                        ...
                        attrs="{'invisible':
                         [('x_checklist_task_instance_ids', '=', [])]}">
                            <field string=" " name="x_checklist_progress_rate"
                            class="o_button_icon"
                                widget="percentpie" nolabel="1"/>
                            <div class="o_stat_info">
                                <span class="o_stat_text" title="%s">%s</span>
                            </div>
                        <field name="x_checklist_task_instance_ids" invisible="1"/>
                    </button>
                    """ % (checklist['name'], checklist['name'])))
                    res['arch'], xfields = self.env['ir.ui.view']. \
                        postprocess_and_fields(root, model)
    ...
    def write(self, vals):
        ...
        checklists_to_recompute.with_context(
            do_not_compute_progress_rates=False)._compute_progress_rates()
        return res
