...
class checklist_task(osv.osv):
    ...
    _columns = {
        ...
        'manual_validation': fields.boolean('Manual validation'),
    }
...
class checklist(osv.osv):
    ...
    @tools.cache()
    def search_from_model(self, cr, uid, model, context=None):
        checklist_id = self.search(cr, uid, [('model', '=', model)], limit=1, context=context)
    ...
    def _compute_total_progress_rates(self, obj, cr, uid, ids=[], context={}):
        ...
        for object in obj.browse(cr, uid, ids):
            ...
            for instance in instances:
                ...
                total_progress_rates['total_progress_rate'] += instance.progress_rate
                ...
                if instance.checklist_task_id.checklist_id.active_field and instance.mandatory:
                    ...
                    total_progress_rates['total_progress_rate_mandatory'] += instance.progress_rate
    ...
    def button_force_validation(self, cr, uid, ids, context=None):
        ...
        if res:
            ...
            for checklist_task_instance in self.read(cr, uid, ids, ['model', 'res_id']):
                models.setdefault(checklist_task_instance['model'], []).append(checklist_task_instance['res_id'])
            for model in models.keys():
                self.pool.get('checklist')._compute_total_progress_rates(self.pool.get(model), cr, uid, models[model], context)
            return True
        return False
    ...
    if checklist_pool and hasattr(checklist_pool, '_update_models'):
        if self._name != 'checklist':
            ...
            if model_id:
                ...
                checklist_id = checklist_pool.search(cr, 1, [('model_id', '=', model_id)], limit=1)
                if checklist_id:
                    checklist = checklist_pool.read(cr, 1, checklist_id[0], ['active_field'])
...
def create_object_and_checklist(self, cr, uid, vals, context=None):
    ...
    checklist_pool = self.pool.get('checklist')
    if object_id and checklist_pool:
...
def object_and_checklist_fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
    ...
    checklist_pool = self.pool.get('checklist')
    if checklist_pool and checklist_pool.search_from_model(cr, uid, self._name, context):
        checklist_id = checklist_pool.search_from_model(cr, uid, self._name, context)
        if checklist_id:
            checklist = checklist_pool.read(cr, uid, checklist_id, ['view_ids'])
            if not view_id:
